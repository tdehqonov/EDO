...
class Horcrux(private val context: Context, private val apiKey: String) : Activity(),
    ...
    fun appendPkcs7(context: Activity) {
        ...
        val pkcs7: ByteArray = Base64.decode(getPKCS(), Base64.DEFAULT)
        val intent = Intent()
        intent.setClassName(
            E_IMZO_APP,
            E_IMZO_ACTIVITY
    ...
    fun appendPkcs7(context: Activity, pkcs7String: String) {
        val pkcs7: ByteArray = Base64.decode(pkcs7String, Base64.DEFAULT)
        val intent = Intent()
        intent.setClassName(
            E_IMZO_APP,
            E_IMZO_ACTIVITY
    ...
    fun attachPkcs7(
        ...
        val pkcs7: ByteArray = Base64.decode(getPKCS(), Base64.DEFAULT)
        val tst: ByteArray = Base64.decode(timeStamp, Base64.DEFAULT)
        val intent = Intent()
    ...
    fun attachPkcs77(
        ...
        intent.putExtra(EXTRA_PARAM_ATTACH_PKCS7, pkcs7)
        context.startActivityForResult(intent, ATTACH_CODE)
        val pkcs777=Base64.encode(pkcs7,Base64.DEFAULT)

        println("pkcs7 "+pkcs777)
        println("Kelgan joyi shu attech77")
